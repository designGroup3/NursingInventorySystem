...
if(isset($_SESSION['id'])) {
    ...
    $sql = "SELECT `Serial Number` FROM inventory WHERE Id = '$id';";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_array($result);
    ...
    $sql2 = "SELECT `Number in Stock` FROM inventory WHERE `Id` = '".$id."';";
    ...
    $sql = "UPDATE inventory SET `Number in Stock` = ".$newStock.", `Last Processing Date` = '".$today."', `Last Processing Person` = '" . $uid . "' WHERE `Id` = '".$id."';";
    $result = mysqli_query($conn, $sql);
