...
if(isset($_SESSION['id'])) {
    include 'dbh.php';
    echo "<head>
              <Title>Search Clients Results</Title>
          </head>
          <body>
              <div class=\"parent\">
                  <button class='help' onclick=\"window.location.href='./UserManual.pdf#page=27'\">
    ...
    if($last == "" && $first == "" && $ext == "" && $email == "" && $office == ""){
        echo "<h3 style='text-align: center'>Please fill out at least 1 search field.</h3><br>
              <div style='text-align: center'>
                  <input onclick=\"window.location.href='searchClientsForm.php';\" class='btn btn-warning' value='Back'>
              </div>";
        exit();
    }
    if($last !== "")
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        if ($acctType == "Admin" || $acctType == "Super Admin") {
            echo "<td>
                      <a href='deleteClient.php?number=$row[Number]&last=$row[Last]&first=$row[First]'>Delete
                  </td>";
        }
        echo "</tr>";
    }
